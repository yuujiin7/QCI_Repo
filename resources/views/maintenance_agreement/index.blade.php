        ...
        <div class="container mx-auto">
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden display" id="SRTable">
                    ...
                    <tbody>
                        @foreach($maintenance_agreements as $ma_reports)
                        <tr class="bg-white border-b text-black hover:bg-gray-50">
                           
                            <td class="py-3 px-6">{{ $ma_reports->sr_number }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->customer_name }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->address }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->engineer_assigned }}</td>
                            <td class="py-3 px-6">{{ date('h:i A', strtotime($ma_reports->start_time)) }}</td>
                            <td class="py-3 px-6">{{ date('h:i A', strtotime($ma_reports->end_time)) }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->total_hours }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->isCompleted }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->machine_model }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->machine_serial }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->product_number }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->part_number }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->part_qty }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->part_desc }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->part_usage }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->action_taken }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->pending }}</td>
                            <td class="py-3 px-6">{{ $ma_reports->remarks }}</td>
                            <td class="py-3 px-6 text-center">
                                <div class="flex justify-center space-x-2">
                                    <a href="/generate-pdf/{{ $ma_reports->id }}" class="bg-blue-600 text-white px-4 py-1 rounded hover:bg-blue-700" target="_blank">
                                    ...
                                    <a href="/service-report/{{ $ma_reports->id }}" class="bg-blue-600 text-white px-4 py-1 rounded hover:bg-blue-700" ><svg class="w-6 h-6 text-white-800 dark:text-gray" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m14.304 4.844 2.852 2.852M7 7H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-4.5m2.409-9.91a2.017 2.017 0 0 1 0 2.853l-6.844 6.844L8 14l.713-3.565 6.844-6.844a2.015 2.015 0 0 1 2.852 0Z"/>
                                        </svg>
                                    </a>
                                    <form action="/service-report/{{ $ma_reports->id }}" method="POST" class="inline" id="deleteForm">
                                        @csrf
                                        @method('DELETE')
